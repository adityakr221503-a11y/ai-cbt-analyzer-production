...
/* =====================================================
   RB_AUTO_IMPORT_TEST180_V1
   Seed the persistent Ranker bank from the existing
   TEST180 project source when the local bank is empty.
   Does NOT touch PDF->CBT storage.
===================================================== */
(function(){
  try{
    if(!window.RankBoosterQuestionBank) return;
    if(window.RankBoosterQuestionBank.count() > 0) return;

    fetch("./test180-questions.json")
      .then(function(res){
        if(!res.ok) throw new Error("TEST180 HTTP "+res.status);
        return res.json();
      })
      .then(function(data){
        ...
        var existing=window.RankBoosterQuestionBank.getAll();
        var seen=new Set(existing.map(function(q){
          return String(q.id||"");
        }));

        src.forEach(function(q,i){
          var question=Object.assign({},q,{
            id:q.id||("TEST180-"+(i+1)),
            ...
            subTopic:q.subTopic||"",
            concept:q.concept||"",
            source:q.source||"TEST180 Ranker"
          });

          if(
            !seen.has(String(question.id)) &&
            question.options.length>=2 &&
            (question.question||question.image||question.imageUrl||
             question.data||question.dataUrl)
          ){
            window.RankBoosterQuestionBank.add(question);
            seen.add(String(question.id));
          }
        });

        console.log(
          "🏆 Ranker Question Bank auto-imported:",
          window.RankBoosterQuestionBank.count()
        );

        window.dispatchEvent(
          new CustomEvent("rankerQuestionBankReady")
        );
      })
      .catch(function(e){
        console.warn("Ranker TEST180 auto-import skipped:",e);
      });
  }catch(e){
    console.warn("Ranker auto-import failed:",e);
  }
})();
